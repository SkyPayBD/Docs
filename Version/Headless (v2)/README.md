...
---
> 📖 **This document is only for the Headless API v2.**
> For v1 Hosted Checkout and the complete API Reference, see the separate document or visit [skypaybd.top/docs](https://skypaybd.top/docs).
---
...
Binance verification works completely differently from MFS.
**How it works (in simple terms):**
- Unlike MFS (bKash, Nagad, etc.), where an SMS arrives on the Android phone and gets matched, that doesn't happen with Binance
- After paying via Binance, the customer copies their **Order ID** from their Binance app and provides it
- SkyPay verifies that Order ID **automatically on the server side** — neither the customer nor you need to do anything extra
- Verification happens instantly — there is no SMS latency
> The customer opens the Binance app → sends the specified USDT → copies the Order ID and provides it to your interface → you verify it → done.
**No Binance credentials from the merchant need to be provided in this document — those only need to be set up once in the SkyPay Dashboard.**
**Endpoint:**
```
POST https://core.skypaybd.top/api/v2/payment/verify
```
...
> - The customer must send the **`amount_usdt`** value given in the `/create` response (tolerance: ±0.0005 USDT)
> - Only **USDT** is accepted — no other currency will work
> - The customer must send to the **Binance Receiving ID** shown in the `/create` response
> - An Order ID cannot be used more than once
...
```
> This latency only applies to **MFS channels** (bKash, Nagad, Rocket, Upay). Binance verification is instant — there is no SMS latency.
---
## 🔒 Security Rules
